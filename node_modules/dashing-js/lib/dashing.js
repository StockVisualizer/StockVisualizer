...
module.exports.Dashing = function Dashing() {
    ...
    app.get('/events', dashing._protected, function(req, res) {
        ...
        // Generate a random id to identify the established connection
        var id = new Date().getTime().toString() + Math.floor(Math.random() * 1000).toString();
        var conn = {
            id: id,
            // set the default ticker symbol to AAPL
            symbol: "AAPL",
            points: [],
            send: function(body) {
                res.write(body);
        ...
        // Store the connection id as the key in the connections hash
    ...
    app.put('/search', function(req, res) {
        // Change the symbol in that connection to the symbol searched for
        connections[req.body.connectionID].symbol = req.body.symbol;
        connections[req.body.connectionID].points = [];
        res.send(200);
    });
    ...
    // An example of the id parameter is 'price'
    function send_event(id, body, identifier) {
        body.id = id;
        body.connectionID = identifier;
        body.updatedAt = Date.now();
        var event = format_event(body);
        history[id] = event;
        connections[identifier].send(event);
    }
    ...
    // Load jobs files, only load jobs when the market is open (i.e. between
    // Mon and Fri from 9:30 AM EST to 4:00 PM EST)
